        ...
        for (auto x : seq) {
            if (prev_is_hat) {
                ...
                if (t != -1) {
                    ...
                    puts(seq.c_str());
                    assert(0 && "unknown char in pileup");
                }
            }
        }
    return ret;
...
PyObject* gen_phase_pipe(PyObject* args, int phase) {
    ...
    sprintf(cmd, "/AxBio_share/software/samtools-1.16/bin/samtools mpileup -r %s:%d-%d -B -q0 -Q0 -aa %s", ctg, start, end, fn);
    auto pipe = popen(cmd, "r");
    
    int nr = end - start + 1;
	nr = (nr-2) / WINDOW_SIZE;
	npy_intp dim = FEAT_DIM[phase] * WINDOW_SIZE * nr;
    auto ret = new int[dim]; //会去掉不够100bp的reads
    memset(ret, 0, dim*sizeof(int));
...
PyMODINIT_FUNC PyInit_parse_pileup(void) {
    PyObject *module;
    ...
    // else
    //     module = Py_InitModule3("parse_pileup",
